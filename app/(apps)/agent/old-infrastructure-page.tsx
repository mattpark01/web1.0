...
export default function AgentInboxPage() {
  const [allAgents] = useState<Agent[]>([])
  ...
  const statusConfig = useMemo(() => ({
    'running': { 
      ...
      count: allAgents.filter(a => a.status === 'running').length
    },
    'idle': { 
      label: 'Idle', 
      icon: Clock,
      ...
      bgColor: 'bg-blue-500',
      count: allAgents.filter(a => a.status === 'idle').length
    },
    'stopped': { 
      label: 'Stopped', 
      icon: Archive,
      ...
      bgColor: 'bg-gray-500',
      count: allAgents.filter(a => a.status === 'stopped').length
    },
    'error': { 
      label: 'Error', 
      icon: AlertCircle,
      ...
      bgColor: 'bg-red-500',
      count: allAgents.filter(a => a.status === 'error').length
    }
  }), [allAgents])
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center max-w-md">
              ...
              <p className="text-muted-foreground mb-6">
                Spawn your first agent to get started with intelligent automation.
              </p>
              
              <div className="space-y-4">
                <Button
                  onClick={() => {
                    // Trigger command palette with spawn agent
                    ...
                    // After a brief delay, navigate to spawn agent
                    setTimeout(() => {
                ...
                <div className="flex items-center justify-center gap-4 text-xs text-muted-foreground">
                  ...
                  <div className="flex items-center gap-1">
                    <span>Type "spawn" to create</span>
                  </div>
                </div>
              </div>
            </div>
